...
- **D1 — REVISED (2026-07-02, implemented): ALL block types mappable, Feed Me-style.** The mapping carries one sub-mapping tree PER block type under a `blocks.<typeHandle>.{fields,nativeFields}` channel (mirroring Feed Me's stored shape), and the builder shows every block type's card at once. Each type's children zip independently; output blocks are GROUPED BY TYPE in the field's declared type order (never interleaved — a per-row discriminator node remains a follow-up). The `newN` key counter runs continuously across types.
...
- **D5 — REVISED (2026-07-02, implemented): FeedMe converter now CONVERTS Matrix `blocks`.** Since the stored shape was aligned with Feed Me's (`blocks.<type>.fields`), conversion reuses the existing recursive `convertMappings()` per type (separator/sentinel/default handling included); the drop-warning is gone. Only no-op block mappings are skipped silently.
