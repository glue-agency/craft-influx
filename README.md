...
Feed Me keeps blocks in the feed's order (it walks the feed and sorts on the array index in each node path), so a converted Matrix picks the block source that reads the same shape rather than the grouped default — the list node moves onto the Matrix row and the child paths are rebased onto a list item. Two shapes can't be read that way and stay grouped with a warning: block types mapped out of different feed nodes, and several block types sitting flat under one list, which is the case Feed Me itself resolves by attributing a shared child handle to whichever type was configured first.
...
- **`Matrix`** — maps a remote sub-array to blocks, one child-mapping tree per block type. A **block source** setting says where they come from and, with it, whether the feed's own block order survives. The default *grouped* reads a list per block type off absolute paths (`seasons.year` beside `directors.name`) and emits one whole type after another — the only source that can build blocks out of unrelated parts of an item, and the only one whose output order is the field's rather than the feed's. The other three read one list node and emit a block per item in feed order, with sub-field paths relative to a list item, differing only in how an item names its block type: not at all (one type), by its own key (`{"text": {…}}`), or by a discriminator node (`{"type": "text", …}`) — the last two matching a per-type feed key that defaults to the Craft handle. Reading per item is also what keeps blocks of the *same* type aligned: collapsing a list drops nulls, so under *grouped* a sub-field missing from one block shifts every later value of it up a block. Every sync still fully replaces the field's blocks from the feed (no per-block merge yet).
...
- `integrations/verbb/tablemaker` — a field strategy for [Table Maker](https://github.com/verbb/tablemaker). Its columns are per-entry content, not field settings, so the row takes one source node holding the whole table and the feed supplies the columns with the values. That means the feed has to speak a fixed shape: **[the format is documented in the integration's own README](src/integrations/verbb/tablemaker/README.md).**
...
- [ ] Matrix per-block merge (today every sync fully replaces a Matrix field's blocks — as do Table and Addresses). Ordering is done: a Matrix's block source decides it, and the three list sources keep the feed's.
- [ ] Relative node suggestions for a list-sourced Matrix. The node select only ever discovered absolute paths, so a list source's sub-field paths are typed rather than picked (the select does accept custom values).
