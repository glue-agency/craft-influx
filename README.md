...
- `integrations/verbb/tablemaker` — a field strategy for [Table Maker](https://github.com/verbb/tablemaker). Unlike Craft's own Table field, its columns are per-entry content rather than field settings, so there is nothing to derive a mapping card from: the operator declares the columns in the mapping (heading, type, and the width/align sub-columns the field enables), and they're written alongside the rows on every sync. From there it behaves like the `Table` strategy — one sub-mapping per declared column, index-zipped into rows. Dropdown columns aren't offered, having an option list nothing here can declare. The Vue control lives with the strategy (`resources/`) and is registered into the builder through the `@integrations` alias.
